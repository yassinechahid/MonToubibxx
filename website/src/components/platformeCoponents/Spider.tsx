...
interface SpiderProps extends Omit<ServiceCardProps, "icon" | "alt"> {
  centered?: boolean;
  icon?: string;
  arrow?: string;
  link?: string;
...
export default function Spider({
  ...
  link,
  alt = "icon",
  className = "", // Added this line with default value
}: SpiderProps) {
  return (
    <motion.div
      ...
      // Updated class name with template literal to include className prop
      className={`flex flex-col z-10 bg-light-surfaceContainerLowest rounded-2xl p-4 gap-3 w-full h-full shadow-[0px_1px_3px_0px_rgba(0,0,0,0.3),_0px_4px_8px_3px_rgba(0,0,0,0.15)] cursor-pointer ${className}`}
    >
      <div
        className={`flex z-10 flex-col gap-2 flex-1 ${
